...
const char* instructions[] = {
    ...
    "LDH [a8], A", "POP HL", "LD [C], A", "NUL", "NUL", "PUSH HL", "AND d8", "RST 20H", 
    "ADD SP, e8", "JP [HL]", "LD [a16], A", "NUL", "NUL", "NUL", "XOR d8", "RST 28H", 
    "LDH A, [a8]", "POP AF", "LD A, [C]", "DI", "NUL", "PUSH AF", "OR d8", "RST 30H", 
...
Breakpoint::Breakpoint(word address) {
    Breakpoint::address = address;
    Breakpoint::type = 0;
}
bool Breakpoint::match(CPU& cpu) {
    return (*cpu.R.pc) == address;
}
...
void print_instruction(CPU& cpu) {
    cpu.prefetch();
    if (cpu.opcode == 0xCB) {
        std::cout << instructions[0x100 + cpu.working_byte] << std::endl;
    }
    else {
        std::cout << instructions[cpu.opcode] << std::endl;
    }
}
    ...
    while (state != DBG_END) {
        if (state == DBG_RUNNING) {
            ...
            for (auto &&b : breakpoints) {
                if (b.match(cpu)) {
                    state = DBG_PAUSED;
                    std::cout << std::endl << "Paused execution @ PC=" << COUT_HEX_WORD_DS(b.address) << std::endl;
    ...
    while (state != DBG_END) { // main loop
        ...
        if (token == "list" || token == "lb" || token == "l") { // List breakpoints
            for (word i = 0; i < b.size(); i++) {
                std::cout << i << " - @ PC=" << COUT_HEX_WORD_DS(b[i].address) << std::endl;
            }
        } else
        //--------------------------------------------------------------
